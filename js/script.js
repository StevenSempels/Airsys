...
$(window).on('load', function() {
	...
	try {
		...
		$container.isotope({
			...
			getSortData: {
				date: function( $elem ) {
					return parseInt(String($elem.attr('data-date')).replace(/-/g, 10));
				},
	...
	$(container).each(function(){
		...
		for (var i = 0; i < height_items.length; i++) {
			$this.children('.otw_portfolio_manager-horizontal-item[data-row="'+ (i + 1) +'"]').each(function(){
				...
				$(this).find('.otw_portfolio_manager-portfolio-media.otw_portfolio_manager-format-image').css( {'width': new_width + 'px', 'height': parseInt(height_items, 10[i]) });
			});
		}
